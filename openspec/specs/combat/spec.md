...
## Purpose
Defines the rules by which attacks are resolved — accuracy rolls, damage calculation, target selection, multi-projectile weapons, and shield behaviour.
...
All damage MUST use the mitigation formula.
...
Weapons with `ProjectilesPerRound > 1` MUST spawn the correct number of projectiles per fire event. The firing mode (sequential or simultaneous) is determined by the `SequentialFire` field in `weapons.json` — independent of `DeliveryType`, which is a spatial descriptor only.

#### Scenario: Sequential (burst) fire — SequentialFire = 1
- **WHEN** a weapon with `SequentialFire = 1` fires (e.g., Machinegun A, Missile Launcher A)
...
- **THEN** it begins in the active state, providing its `Defense` bonus to the **whole mech** for all incoming hits
- **AND** the active window lasts `SHIELD_ACTIVE_DURATION_MS` (currently 5000 ms; TODO: pull from weapon data)
- **AND** the active window is visible via a HUD bar draining and a body glow effect
...
- **WHEN** a melee enemy reaches the Mech (a circular target)
- **THEN** the attack range check uses `center_distance - target.radius`
